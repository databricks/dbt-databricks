...
`gap set as met/gap/n-a per rubric item; changelog requirement for test-only changes; PR write-mode classification; harness-worktree preference and .agents fallback; exact head repository and branch target; moved-head protection; selection, commit, and push approval gates`
...
- A separate worktree supplied by the active agent harness is preferred;
  `.agents/worktrees/pr-<N>` is the fallback when execution starts in the
  shared primary checkout. Reports remain under the primary checkout's
...
target and moved-head protection → p3 and p5; harness and fallback worktree
selection → p1 and p6; selection and approval gates → p3–p6. Add a case before
introducing a new decision dimension.
