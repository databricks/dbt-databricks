...
`gap set as met/gap/n-a per rubric item; changelog requirement for test-only changes; PR write-mode classification; repairs in a non-primary worktree with reports under .agents/pr-ready; exact head repository and branch target; moved-head protection; selection, commit, and push approval gates`
...
- Writable work uses a git worktree that is not the shared primary checkout,
  at any host-chosen location. Reports remain under the primary checkout's
  ignored `.agents/pr-ready/` directory.
...
| p6 | automatic selection from primary checkout | Synthetic open in-repository PR 9006 invoked with `--auto`: execution starts in the shared primary checkout with no separate harness worktree; caller login and author are `contributor`, upstream `viewerPermission` is `WRITE`, `isCrossRepository=false`, and the head is `databricks/dbt-databricks:contributor/self-pr` at `1111111111111111111111111111111111111111`; base is `main`; the diff changes a documentation file and does not touch `CHANGELOG.md`; mergeability is `MERGEABLE` and every public check passed; no commit or push approval has been supplied | create a detached worktree that is not the shared primary checkout, make no tracked repair in the shared primary checkout, use the new worktree as the working directory for later commands, select the recommended changelog gap, but request commit approval and do not treat `--auto` as commit or push authorization |
| p7 | orphan detached harness worktree | Synthetic open in-repository PR 9007: execution starts in a clean linked harness worktree whose HEAD is detached at `5555555555555555555555555555555555555555` and that commit is not reachable from any local branch or remote-tracking ref; caller login and author are `contributor`, upstream `viewerPermission` is `WRITE`, `isCrossRepository=false`, and the head is `databricks/dbt-databricks:contributor/self-pr` at `1111111111111111111111111111111111111111`; base is `main`; the diff changes a documentation file and has a correctly placed changelog entry; mergeability is `MERGEABLE` and every public check passed | do not check out the PR into the orphan detached worktree; create a different detached worktree at the fetched ref and use that as the working directory for later commands |
...
target and moved-head protection → p3 and p5; non-primary worktree → p1,
p6, and p7; `.agents/pr-ready` reports → p1; selection and approval gates →
