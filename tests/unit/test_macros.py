...
class TestSparkMacros(unittest.TestCase):
    ...
    def setUp(self):
        self.jinja_env = Environment(loader=FileSystemLoader('dbt/include/databricks/macros'),
    ...
    def test_macros_create_table_as_hudi_options(self):
        ...
        sql = self.__run_macro(template, 'databricks__create_table_as', False, 'my_table', 'select 1 as id').strip()
        ...
        self.config['options'] = {'primaryKey': 'id'}
        sql = self.__run_macro(template, 'databricks__create_table_as', False, 'my_table', 'select 1 as id')
