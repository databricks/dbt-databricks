...
class TestAdapterCapabilities:
    ...
    def test_has_dbr_capability_method(self, adapter):
        ...
        mock_conn = Mock()
        mock_conn.has_capability = Mock(side_effect=lambda cap: cap.value != 'invalid_capability' if hasattr(cap, 'value') else False)
        ...
        with patch.object(
            ...
            assert adapter.has_dbr_capability('json_column_metadata') == True

            # Test invalid capability name (should return False for unknown capabilities)
            assert adapter.has_dbr_capability('invalid_capability') == False
