...
# Test 7: composite unique_key with merge_actions_explicit
# Two-column key (id, type). Explicit actions update name when action='u', insert when new.
merge_explicit_composite_key_expected = """id,type,name
1,a,updated
2,b,original
3,c,inserted
"""
...
{{ config(
    ...
    merge_actions_explicit=\"\"\"
        ...
        when not matched
            then insert (id, type, name) values (s.id, s.type, s.name)
    \"\"\",
) }}

{% if not is_incremental() %}

select 1 as id, 'a' as type, 'original' as name
union all
select 2 as id, 'b' as type, 'original' as name

{% else %}

select 1 as id, 'a' as type, 'updated' as name, 'u' as action
union all
select 3 as id, 'c' as type, 'inserted' as name, 'i' as action

{% endif %}
"""

# Test 8: full-refresh with merge_actions_explicit — separate model with no ambiguous NULL/empty
# After --full-refresh only the non-incremental branch rows remain.
merge_explicit_full_refresh_model = """
{{ config(
    materialized = 'incremental',
    unique_key = 'id',
    incremental_strategy = 'merge',
...
select 1 as id, 'original_a' as name
union all
select 2 as id, 'original_b' as name

{% else %}

select 1 as id, 'updated_a' as name
union all
select 3 as id, 'new_c' as name

{% endif %}
"""

merge_explicit_full_refresh_expected = """id,name
1,original_a
2,original_b
"""
