...
- Back-quote column identifiers generated by the `insert_overwrite`, `append`, and `delete+insert` incremental strategies so that non-ASCII column names (e.g. Japanese) no longer fail with `INVALID_IDENTIFIER` on subsequent runs ([#1595](https://github.com/databricks/dbt-databricks/pull/1595) resolves [#1594](https://github.com/databricks/dbt-databricks/issues/1594))
