...
- Support the connector's Rust kernel backend via `connection_parameters: {use_kernel: true}` for SQL warehouses (requires `databricks-sql-connector[kernel]` on Python 3.10+). The kernel's auth bridge rejects the connector `credentials_provider` dbt uses on the default path, so the adapter forwards the raw credentials the kernel understands — personal access token or Databricks OAuth (M2M/U2M) — letting the kernel own the token lifecycle, including OAuth refresh. Azure service principals have no kernel-native flow, so they fall back to a pre-resolved bearer token (logged with a warning) that does not auto-refresh; use a PAT or Databricks OAuth, or the default backend, for long-lived Azure connections ([#1576](https://github.com/databricks/dbt-databricks/pull/1576))
