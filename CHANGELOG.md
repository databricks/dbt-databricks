...
- Support the connector's Rust kernel backend via `connection_parameters: {use_kernel: true}` for SQL warehouses (requires `databricks-sql-connector[kernel]` on Python 3.10+). PAT and Databricks OAuth (M2M/U2M) are supported natively; Azure service principals fall back to a pre-resolved bearer token that does not auto-refresh ([#1576](https://github.com/databricks/dbt-databricks/pull/1576))
