...
- Fix unnamed primary keys churning on every incremental run: Databricks auto-names an unnamed PK, so the model's `name=None` never matched the catalog and reconciliation issued `DROP CONSTRAINT ... CASCADE` + re-add each run, silently dropping dependent FKs. Building on [#1552], a PK is now matched on columns alone (a table has exactly one); a deliberate rename of a *named* PK is still reconciled. dbt also now warns when a foreign key is declared without a `name`, since unnamed FKs churn the same way — assign one to avoid it. ([#1554](https://github.com/databricks/dbt-databricks/pull/1554) closes [#1333](https://github.com/databricks/dbt-databricks/issues/1333))
