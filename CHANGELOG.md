...
### Fixes
- Warn when a column is documented in a model's `schema.yml` but is not present in the materialized relation, instead of silently skipping it. Helps surface typos and stale column documentation. Ports the behavior added in dbt-adapters ([dbt-adapters#1684](https://github.com/dbt-labs/dbt-adapters/pull/1684) closes [dbt-adapters#1690](https://github.com/dbt-labs/dbt-adapters/issues/1690)).
