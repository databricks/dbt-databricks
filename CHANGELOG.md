...
- Bump `databricks-sql-connector` to 4.2.7 and tighten its ceiling back to patch level (`<4.3.0` → `<4.2.8`), so future patch releases require an intentional bump rather than being admitted silently. ([#1497](https://github.com/databricks/dbt-databricks/pull/1497))
