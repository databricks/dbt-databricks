{#-- CREATE embeds an explicit column list (Databricks user-specified schema).
    ...
    REFRESH fails — recreate instead (#1359). The model's query text is unchanged in
    that scenario, so the QueryProcessor component of get_configuration_changes cannot
    detect it. Names only: type-label variance across DESCRIBE paths would cause
    spurious recreates. Streaming tables (#1303) are out of scope here: locking
    columns on CREATE is a separate design question. --#}
